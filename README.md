...
> **📦 归档说明**：本仓库已归档，不再跟随主项目更新。最新版的 [**LLM-Manager**](https://github.com/lingyezhixing/LLM-Manager) 可完美承接本项目的全部任务（模型托管 + 统一 API 网关 + 管理界面）。本仓库作为核心调度模块存档保留——其进程调度、资源管理与透明代理逻辑已基本完善，适合基于此核心进行二次开发。
